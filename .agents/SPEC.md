...
Windows 版本固定以 File-only Mode 運作，不可啟動 bundled backend 或嘗試 ROS
discovery。Windows release 必須能直接啟動 GUI、載入／拖放 graph JSON，並保留
搜尋、篩選、graph 互動及匯出功能。
Windows portable EXE 應使用兼顧檔案大小與啟動速度的壓縮格式，避免以極高壓縮率
增加每次啟動的解壓等待時間。
